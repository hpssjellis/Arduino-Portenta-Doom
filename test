...
//   D2  Joystick SW     active LOW (INPUT_PULLUP): pressed = LOW → KEY_RSHIFT (run)
//   D3  Fire button     active LOW (INPUT_PULLUP): pressed = LOW → KEY_FIRE
//   D4  Use button      active LOW (INPUT_PULLUP): pressed = LOW → KEY_USE
//   D5  Enter button    active LOW (INPUT_PULLUP): pressed = LOW → KEY_ENTER
//
...
static void myUpdateJoystick()
{
    ...
    int curRun   = (digitalRead(D2) == LOW) ? 1 : 0;
    int curFire  = (digitalRead(D3) == LOW) ? 1 : 0;
    int curUse   = (digitalRead(D4) == LOW) ? 1 : 0;
    int curEnter = (digitalRead(D5) == LOW) ? 1 : 0;
...
void DG_Init()
{
    ...
    pinMode(D2, INPUT_PULLUP);  // joystick SW  → KEY_RSHIFT (run)
    pinMode(D3, INPUT_PULLUP);  // Fire button  → KEY_FIRE
    pinMode(D4, INPUT_PULLUP);  // Use button   → KEY_USE
    pinMode(D5, INPUT_PULLUP);  // Enter button → KEY_ENTER
}
